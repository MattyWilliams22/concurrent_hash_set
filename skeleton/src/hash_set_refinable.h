...
class HashSetRefinable : public HashSetBase<T> {
 public:
  explicit HashSetRefinable(size_t capacity) : table_(capacity), size_(0), capacity_(capacity), resizing_flag_(false)  {
  ...
  bool Add(T elem) final {
    ...
    while (true) {
      // busy wait while resizing this flag allows mutual ex as in expert from textbook
      if (resizing_flag_.load()) {}

      // standard procedure (as from striped) for getting current capacity etc...
      // However, we use a shared mutex here to allow many threads to read the locks_
      // list but prevent it when we are writing in resize
      ...
      // someone has started resizing or has resized before we managed to acquire the lock
      // hence we loop again and re calculate which bucket and lock
      // If predicate pass it means no one is resizing or has resized while waiting to lock
      // our information is in-date, and we can proceed adding to the bucket
 ...
 private:
  ...
  void resize(size_t old_capacity) {
    ...
    bool desired = true;
    bool result =
        resizing_flag_.std::atomic<bool>::compare_exchange_strong(expected, desired);

    // Someone is already resizing so unnecessary
    if (result != true) {
      return;
    }
    // checks on old capacity captured when resize is called and returns if that capacity
    // is stale, and a thread beat us to resizing
    if (old_capacity != capacity_.load()) {
      resizing_flag_.compare_exchange_strong(desired, expected);
      return;
    }

    // use lock shared than lock_shared to get a reader lock to the locks_list then
    // block until the locks are unlocked --> this forces thread to get caught flag is being unforced
    locks_list_lock_.lock_shared();
    ...
    // now calling lock on shared lock to get a write lock and mutex on whole list as we are modifying it
